...
class MY_Model extends CI_Model
{
	...
	* 	@param $groupBy = array("age","salary") or string;
	*/
	public function findDataTableOutput($data=null,$where=false,$select=false,$search=false,$join=false,$whereIn=false,$groupBy=false)
	{
		$input = $this->input;
		if ($search) {
			$dataSearch = array();
		...
		$response->recordsTotal = $this->getCount($where,$search,$join,$whereIn,$groupBy,false,true,$select);
		// $response->recordsTotal = $this->getCountAll();
		$response->recordsFiltered = $this->getCount($where,$search,$join,$whereIn,$groupBy,false,true,$select);
		$response->data = $data;
	...
	* @param $orderBy = "id DESC,name ASC"; or array("id DESC","name ASC"); // if true
	* @param $join = array(array("table2","table2.id = table1.id",[LEFT])); // if true
	* @param $whereIn["in"] = array("id" => array(1,3,3,2,4,5));
	* @param $whereIn["not"] = array("id" => array(1,3,3,2,4,5));
	* @param $groupBy = array("age","salary") or string;
	* @param $result_array = if(true){ result array() }else{ result object}
	*/
	public function getAllSelect2($limit=false,$search=false,$where=false,$select=false,$orderBy=false,$join=false,$whereIn=false,$groupBy=false,$result_array=false)
	{
		...
		if ($whereIn) {
			if (isset($whereIn["in"])) {
				$this->db->where_in(array_keys($whereIn["in"])[0],array_values($whereIn["in"])[0]);
			} else if (isset($whereIn["not"])) {
				$this->db->where_not_in(array_keys($whereIn["not"])[0],array_values($whereIn["not"])[0]);
			}
		}
		if ($groupBy) { $this->db->group_by($groupBy); }
		$query = $this->db->get($this->_table);
